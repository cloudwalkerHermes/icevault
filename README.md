...
If `$EDITOR` isn't set in your shell, `sops` will fall back to `vi`. If you're not comfortable in `vi`, set a friendlier editor first: `export EDITOR=nano` (or your preference) before running `manage_secrets.py`. On **Windows**, `manage_secrets.py` and `manage_multiline_secret.py` automatically pick the best available editor when `EDITOR` isn't set: VS Code (`code --wait`) if installed, then Notepad++, then plain `notepad` as a last resort. VS Code is the best Windows experience — set `EDITOR=code --wait` explicitly if you want to force it. If you prefer vim, the `^`-prefixed characters you may see are Windows `\r\n` line endings; run `:set ff=unix` at the vim command line before editing to clear them.
